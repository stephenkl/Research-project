...
__all__ = ['ResNet_SlowFast', 'i3d_slow_resnet50_f32s2_kinetics400',
           ...
           'i3d_slow_resnet101_f16s4_kinetics700', 'i3d_slow_resnet50_f16s4_custom',
           'i3d_slow_resnet101_f16s4_custom']
...
def i3d_slow_resnet50_f16s4_custom(cfg):
    model = ResNet_SlowFast(num_classes=cfg.CONFIG.DATA.NUM_CLASSES,
                            depth=50,
                            pretrained=cfg.CONFIG.MODEL.PRETRAINED,
                            pretrained_base=cfg.CONFIG.MODEL.PRETRAINED_BASE,
    ...
    if cfg.CONFIG.MODEL.PRETRAINED:
        from ..model_store import get_model_file
        state_dict = torch.load(get_model_file('i3d_slow_resnet50_f16s4_kinetics400', tag=cfg.CONFIG.MODEL.PRETRAINED))
        for k in list(state_dict.keys()):
            # retain only backbone up to before the classification layer
            if k.startswith('fc'):
                del state_dict[k]
...
def i3d_slow_resnet101_f16s4_custom(cfg):
    model = ResNet_SlowFast(num_classes=cfg.CONFIG.DATA.NUM_CLASSES,
                            depth=101,
                            pretrained=cfg.CONFIG.MODEL.PRETRAINED,
                            pretrained_base=cfg.CONFIG.MODEL.PRETRAINED_BASE,
    ...
    if cfg.CONFIG.MODEL.PRETRAINED:
        from ..model_store import get_model_file
        state_dict = torch.load(get_model_file('i3d_slow_resnet101_f16s4_kinetics400', tag=cfg.CONFIG.MODEL.PRETRAINED))
        for k in list(state_dict.keys()):
            # retain only backbone up to before the classification layer
            if k.startswith('fc'):
                del state_dict[k]
