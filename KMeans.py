# python KMeans.py --source ./Test_Video/clip002.mp4  --yolo_weights yolov5/weights/crowdhuman_yolov5m.pt --classes 0 --show-vid --save-vid
...
def detect(opt):
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        ############## modified #####################
                        # only allow person class
                        allowed_class_name = names[0]
                        if names[c] == allowed_class_name:
                            if id in track_id_count_dict:
                                track_id_count_dict[id] += 1
                                
                            else:
                                track_id_count_dict[id] = 0
                                detect_img_copy = img_copy[max(0, int(bboxes[1])):max(0, int(bboxes[3])), max(0, int(bboxes[0])):max(0, int(bboxes[2]))].copy()
                                #cv2.imwrite('img_shot/'+str(id)+"_"+str(track_id_count_dict[id])+".jpeg", detect_img_copy)
                                image_lst.append([detect_img_copy,id])
    ...
    # extract the image features from the images
    feature_df = feature_extractor(image_lst, 1)

    print("All features are extracted")

    # build Kmeans model using features
    kmeans_model = KMeans_builder(feature_df, CLUSTER_NUMBER)
    print("KMeans model has been built")
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        ############## modified #####################
                        # change the Person's image box info

                        if c == 0:
                            detect_img_copy = img_copy[max(0, int(bboxes[1])):max(0, int(bboxes[3])), max(0, int(bboxes[0])):max(0, int(bboxes[2]))].copy()
                            new_image = [[detect_img_copy,id]]
                            new_image_feature = feature_extractor(new_image, 0)
                            predict_team_num = KMeans_predict(kmeans_model, new_image_feature)
                            color = person_box_color_after_kmeans(predict_team_num)
                            label = f'{predict_team_num}'
