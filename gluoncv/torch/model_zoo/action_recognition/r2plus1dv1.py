...
def r2plus1d_v1_resnet152_custom(cfg):
    model = R2Plus1D(num_classes=cfg.CONFIG.DATA.NUM_CLASSES,
                     block=Bottleneck,
                     layers=[3, 8, 36, 3],
                     num_segment=cfg.CONFIG.DATA.NUM_SEGMENT,
    ...
    if cfg.CONFIG.MODEL.PRETRAINED:
        from ..model_store import get_model_file
        state_dict = torch.load(get_model_file('r2plus1d_v1_resnet152_kinetics400', tag=cfg.CONFIG.MODEL.PRETRAINED))
        for k in list(state_dict.keys()):
            # retain only backbone up to before the classification layer
            if k.startswith('fc'):
                del state_dict[k]
